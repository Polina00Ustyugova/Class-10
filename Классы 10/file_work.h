...
{
	fstream stream(filename, ios::out | ios::trunc); // открыть для записи
	if (!stream) return -1; 
	int n;
	Pair p;
	cout << "Размер: ";
...
int del_file(const char* filename, Pair t) 
{
	...
	stream.close();
	temp.close();
	remove(filename); // Удаление файла с именем filename
	rename("temp", filename); // Переиминовывание файла
	return i; 
}
...
int add_file(const char* filename, int k, int count) 
{
	...
	while (stream >> p && !stream.eof()) 
	{
		...
		if (i == k) 
		{
			...
			{
				Pair new_Pair;
				cout << "Введите новую пару чисел" << endl;
				cin >> new_Pair;
				temp << new_Pair;
				l++;
			}
...
{
	fstream stream(filename, ios::app); // запись в конец файла
	if (!stream) return -1;
